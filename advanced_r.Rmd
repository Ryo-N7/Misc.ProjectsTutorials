...
Easiest way to make function faster is to let it do less work. Use functions tailored to a more specific type of input/output.

* rowSums(), colSums(), rowMeans(), and colMeans() are faster than equivalent invocations that use apply() because they are vectorised (Section 24.5).
...
Less work if functions given more information via arguments!
...
Avoid method dispatch. Avoid costs in loop by doing method lookup only once. 

S3: call `generic.class()` > `generic()`
S4: `getMethod()` and save to variable, then call that function

Ex. `mean.default()` > `mean()` for small vectors.

X must be numeric vector or errors.

Call `.Internal` directly. No input checking or NA handling. Speed > Safety.

Increase in size: Time spent on computation > finding underlying implementation. Use realistic data for optimization tests.

### 24.4.2: `as.data.frame()`

Coerces each element into data frame and then `rbind()` together. 

## 24.5 Vectorize

Vectorizing: Vectors > scalars. 

* Entire vectors > components of a vector
* Loops in a vectorized function in C > R.

Downside: harder to predict how operations will scale. 

## 24.6 Avoiding copies.

Create a bigger object >>> R allocate space for new object then copy old object to new home. 


Modifying object in a loop can also create a copy, depending on class of x.
